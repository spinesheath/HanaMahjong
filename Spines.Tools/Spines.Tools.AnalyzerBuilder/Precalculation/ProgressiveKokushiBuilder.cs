...
namespace Spines.Tools.AnalyzerBuilder.Precalculation
{
  internal class ProgressiveKokushiBuilder : IStateMachineBuilder
  {
    public ProgressiveKokushiBuilder()
    {
      ...
      var transitions = new int[equivalenceGroups.Count * AlphabetSize].Populate(-1);
      ...
      for (var i = 0; i < normalStates.Count; ++i)
      {
        ...
        for (var c = 0; c < AlphabetSize - 1; ++c)
        {
          ...
          var newTransiton = oldToNewIds[oldTransition] * AlphabetSize;
          transitions[newId * AlphabetSize + c + 1] = newTransiton; // Shift the transitions one character to the back.
        }
